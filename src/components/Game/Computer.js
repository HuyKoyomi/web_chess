...
function Computer() {
  ...
  const handleMove = async (move) => {
    ...
    if (chess.move(move)) {
      ...
      if (chess.in_draw() || chess.in_checkmate()) return;
        ...
        <div id="left-sidebar-wrapper">
          <Table striped bordered hover variant="dark">
            <tbody>
              {inCheck && (
                <tr>
                  <td colSpan="2" id="in-check-text">
                    {turn.charAt(0).toUpperCase() + turn.slice(1) == "White"
                      ? "Trắng"
                      : "Đen"}{" "}
                    đang bị chiếu tướng
                  </td>
              ...
              <tr>
                <td>Lượt đi</td>
                <td>{turn == "white" ? "Trắng" : "Đen"}</td>
              </tr>
              <tr>
                <td>Quân của bạn</td>
                <td>{playerColor == "white" ? "Trắng" : "Đen"}</td>
              </tr>
              <tr>
                <td>Trạng thái</td>
                {matchStatus === "Ongoing" ? (
                  <td>{matchStatus == "Ongoing" ? "Tiếp tục chơi" : ""}</td>
                ) : matchStatus === "You Won" ? (
                  //   <td id="win-status">You Won</td>
                  <td id="win-status">Bạn Thắng</td>
                ) : (
                  //   <td id="lose-status">You Lose</td>
                  <td id="lose-status">Bạn Thua</td>
                )}
              </tr>
            </tbody>
          </Table>
